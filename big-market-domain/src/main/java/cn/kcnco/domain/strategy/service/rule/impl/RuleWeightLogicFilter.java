...
public class RuleWeightLogicFilter implements ILogicFilter<RuleActionEntity.RaffleBeforeEntity> {
     ...
     *
     * @param ruleMatterEnitity 规则物料实体对象
     * @return 规则过滤结果
     */
    @Override
    public RuleActionEntity<RuleActionEntity.RaffleBeforeEntity> filter(RuleMatterEnitity ruleMatterEnitity) {
        log.info("规则过滤-权重范围 userId:{} strategyId:{} ruleModel:{}", ruleMatterEnitity.getUserId(), ruleMatterEnitity.getStrategyId(), ruleMatterEnitity.getRuleModel());

        String userId = ruleMatterEnitity.getUserId();
        Long strategyId = ruleMatterEnitity.getStrategyId();
